...
- **HEAD when this was written**: `682491d5` (74 commits on `main`; **fully pushed to `origin/main`**)
...
| **Alignment dialog** | Custom HTML5-DnD aligner: source-order block layout, RTL grid, drag-merge sources, multi-select target bag with shift-range, per-block × clear, placeholder blocks for UHB source words not yet in the target, lexicon tooltip on every source-word hover (in both the alignment cards AND the verse strip). Other-version chip is clickable to switch which side is being aligned. ULT/UST cells in the verse strip are contentEditable — edits flow through the shared smart-edit pipe. |
