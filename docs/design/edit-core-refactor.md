...
> **Status:** Design proposal for human review. No code changes in this PR.
> **Scope:** `web/src/lib/replace.ts` (`smartEditVerse` / `smartReplaceVerse` internals).
> **Relationship to other work:** This is one of four follow-ups to the 2026-06-18 alignment-loss
> investigation. It is *complementary* to the hardened #227 guard (a separate PR) — the guard is a
> fail-closed gate that refuses to *persist* a bad delta; this proposal is about making the edit
> engine *produce* a good delta by construction so the guard rarely has to fire.
...
shape) and keep inline `\zaln` storage; swap the *internal* range-surgery engine for an
unmerge/diff/merge round-trip that reassembles surviving alignments onto the new target string by
(surface text, occurrence) — and if reassembly cannot reconstruct the verse, **keep the prior verse
content unchanged and flag "needs re-align" — never flatten.**
...
The swap sits entirely behind the unchanged `smartEditVerse` API, so it can be staged without
touching call sites (`Shell.tsx`, `ScriptureColumn.tsx`, `BookView.tsx`, `DocColumn.tsx`,
`FindReplaceOverlay.tsx`, `highlight.ts`, `usfm.ts` all call it the same way).

**Phase 0 — Port + parity harness (no behaviour change).**
Port `unmerge`/word-diff/`merge` into `web/src/lib/alignmentReassembly.ts`. Do not wire it into
`smartEditVerse` yet. Add a parity test that runs *both* the new engine and the current tiers over
the full `replace.test.mjs` corpus + the ~12k-edit synthetic integration harness
(`memory/project_edit_engine_adversarial_findings.md`) and reports per-case milestone deltas.
**Acceptance:** new engine ≥ current engine on surviving-milestone count for every case; zero cases
where new < current by more than the gatewayEdit-expected local drop.

**Phase 1 — Wire in behind a flag, fall back to tiers.**
`smartEditVerse` calls the new engine first; if it fails its self-check, fall through to the existing
tiers (NOT to a flatten), and only if *those* also fail, fail closed (keep prior verse +
needs-re-align). Behind a build/feature flag so prod default is unchanged.
**Acceptance:** typecheck + `npm run build` + full `replace.test.mjs` green with the flag on; 12k
harness all-zero unexpected losses.

**Phase 2 — Canary on real edits.**
Enable the flag in dev / for a small set of books being actively worked. Use the
`analyzeAlignmentDelta` instrumentation already present (`api/src/alignmentDelta.ts` /
`web/src/lib/alignmentDelta.ts`) to *log* (not block) the delta of every save under the new engine.
Drive the app via the Claude-in-Chrome / dev-server flow (`CLAUDE.md` "Browser-driven verification")
to confirm: type a name-spelling edit on 1CH 4:21, save, reload, assert ≥32/33 milestones survive in
D1.
**Acceptance:** over the canary window, zero saves where the new engine produced *more* unexpected
losses than the old, and the 1CH 4:21 / NUM 24 manual cases hold.

**Phase 3 — Default on, retire tiers (optional).**
Flip the flag on by default. Decide, based on Phase-1 parity data, whether to delete the legacy tiers
or keep them as the documented fallback. Keep the fail-closed branch regardless.

**Verification / acceptance bar (all phases):**
1. `npm run typecheck` and `npm run build` clean.
2. `npm --workspace web run test` — all existing `replace.test.mjs` cases green, no edits to existing
   assertions.
3. 12k-edit synthetic harness: zero *unexpected* alignment losses (local single-word drops on a
   word-edit are expected and allowed).
4. Manual browser canary on 1CH 4:21 and a NUM 24 verse: survivors persisted to D1, confirmed on
   reload.
5. No path produces a whole-verse flatten — flatten is replaced by fail-closed needs-re-align.
...
1. **Port vs. depend** — do we accept re-introducing a `word-aligner-rcl` family dependency despite
   the documented Vite/Rollup reason for avoiding it, or commit to the ~3-function port? (§4.1 — I
   recommend port.)
2. **Tier retirement** — after Phase-1 parity, keep the legacy tiers as a documented fallback layer,
   or delete them once the new engine is default? (Affects long-term maintenance surface.)
3. **Fail-closed UX** — when reassembly can't reconstruct, what exactly does the translator see?
   A toast? An inline "needs re-align" badge on the verse? Does it block the text save (keep prior
   content) or save the *text* and only blank the alignment (closer to tC's reset-verse behaviour)?
   This is the one user-facing decision in the design.
4. **Ordering vs. #227** — should this refactor land *before* the #227 predicate is tightened (so the
   tightened guard never false-positives on legacy-engine edits), or after (so the guard catches any
   reassembly bug during canary)? §6.2 assumes they co-design the shared keying.
5. **NUM 24 re-validation** — NUM 24 was never re-tested on current code. Do we want a one-off
   re-test of the *current* engine on NUM 24 as a baseline before the refactor, so Phase-0 parity has
   a real comparand rather than the known-bad old number?
