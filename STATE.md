...
- **Verified — how:** `npm run typecheck` (both workspaces), api unit suite (incl. new lib tests), web unit suite, `npm run build` — all green, re-run after rebasing onto `origin/main` (`2fc0824`, resource-labels PR #10; clean rebase, only additive `api.ts` overlap). **Browser smoke (in-app Claude Browser, worktree wrangler on :8788 serving the built SPA, `ar-bsoj`):** GL gate shows the explainer under English root; panel renders (rail + العربية); Brief save round-trip (audience persisted, version 0→1 CAS); term added via UI (kt/grace→نعمة); CSV import dry-run (verified no write) then apply — multi-rendering concept kt/lord (preferred الرب + forbidden السيد with replacement) + DNT names/yhwh → count 4; export round-trips with Arabic + tw_link intact; UI status chips correct incl. forbidden "use instead: الرب"; Examples tab correct empty state ("0 validated"); Instructions save persisted (version 1→2). Branch pushed to `origin/feat/translation-preferences-panel` (personal fork; verified `git remote -v`).
- **Honesty ledger:** **examples list/revoke not browser-exercised with real validated rows** (this worktree's D1 has none seeded; revoke reuses the already-shipped `validate(0)` endpoint); examples reach the AI only once the context-repo export + assisted mode exist (designed, deferred — blocked on `CONTEXT-REPO-CONTRACT.md`); QA-rules + templates UIs deferred (backend tables exist in 0040); tW key-term seeding deferred; register/terminology actually reaching uw-bt-bot unconfirmed (their steering is undocumented). **Local-dev traps hit this run (lessons):** a peer wrangler on :8787 serves stale main-code while claiming our `npm run dev` also "Ready on 8787" — probe `/api/translation-memory/prefs` to detect, pin a free port (`wrangler dev --port 8788`, serves SPA via `[assets]`); fresh worktrees lack `api/.dev.vars` (copy from main or `/api/auth/dev` 500s `jwt_signing_key_not_configured`); a prod-built SPA never auto-mints dev auth — mint manually via `POST /api/auth/dev` then reload; Claude-Browser coordinate clicks silently miss MUI buttons — use DOM-dispatched clicks + native value setters.
