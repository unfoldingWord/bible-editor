...
2026-07-20 · **TWL ordering now anchors on the TW headword, not the span's first English word (mid-flight, not yet merged).** Team-lead direction: a TW check should ask "how did you translate *house*?", not "how did you translate *and … the house of*?" — so order links by the ULT word carrying the article's headword and ignore the attached particles, "both for ordering and for display in tC's TW checking tool". Display is **tC/Birch's side (tCore 4), explicitly out of scope here**; this repo does the ordering, in BE display *and* export. **Design:** `buildUltSequenceMap` (both mirrors) now stores each alignment span's full English word list (`WordRef{index,text}`) instead of a single index, because the anchor can no longer be chosen while walking — which word wins depends on the row's TW article. Anchor picked at lookup time by `selectAnchor`, three tiers: (1) word matching the TW headword, (2) else, when the span has >1 word, first non-function word, (3) else first word. **Tier 3 is provably the old value** (lowest index in the span) — every pre-existing ordering test passed unmodified, which is the evidence that unmatched rows keep today's position. Ordering also moved to decorate-sort (position computed once per row, not inside the comparator) since headword matching runs morphological variants. **New shared module `twHeadword.ts` (api + web verbatim mirror)** — `generateVariants`/`IRREGULAR_*` extracted out of `twlMatcher.ts` so the suggestion matcher and the ordering code share ONE definition of "does this English word correspond to this headword"; plus `headwordTermsFromTitle`, `matchesHeadword`, `FUNCTION_WORDS`/`isFunctionWord`. Title map plumbed: `GET /api/catalogs` now returns `twTitles` (tw_link→title, ~950 entries on an already-persisted payload), `useCatalogs` localStorage key bumped **v1→v2** (a stale v1 payload has no titles and would silently disagree with the export under stale-while-revalidate), `ResourceColumn` passes it to both `canonicalTwlOrder` call sites; server side loads it via new `twTitles.ts` (`loadTwTitles`) in `exportWorkflow` + `bookReimport`. **Measured on real prod data, 7 books / 5,775 links (RUT OBA MIC ZEC HOS EST JHN):** visible order changes = **0**; anchor moved onto the content word for **45% of rows** (2,626/2,945 multi-word spans); tier1 (real headword match) **86.5%**, tier2 5.7%, tier3 7.4%, unresolved 0.4% (22 JHN rows with no aligned English — pre-existing, treated identically by old and new code). **So the export churn will be negligible — an earlier warning of "large one-time churn" was wrong and is retracted.** Honest read: on measured data this reorders nothing; its value is correct anchor semantics + agreement with what tC will display. The reorder benefit only bites where two links' spans overlap/nest — unit-tested (nested pair flips) but **NOT yet demonstrated on production data**. **Pronouns/auxiliaries added to `FUNCTION_WORDS`** after auditing all 953 en_tw articles (1941 terms): only 5 terms touch the list — `might` (other/mighty, 251 rows) is an exact headword so tier 1 claims it first (zero exposure); `I am Yahweh`/`who talk with spirits`/`be subject to` all improve; only `will of God` (17 rows) mildly regresses (anchors *God* not *will*, since a multi-word term can't match a lone word in tier 1). Lexical verbs (`make`/`made`/`let`) deliberately excluded. This fixed the two anchor cases the first measurement flagged (MIC 1:6 "So I will make"→*make*, MIC 3:5 "then they call out"→*call*), both now regression tests. **Verified:** api typecheck 0 errors, web typecheck 0 real errors, 18 api + 12 web suites green, `twlMatcher.test.mjs` passes unmodified. **NOT verified: no browser smoke test** — blocked on damaged `node_modules` (see next entry). **Open:** whether to run the full 30-book corpus measurement (7 books is ~7%); ship decision. Data-quality nit found: one prod TWL row points at `kt/arcofthecovenant` (typo for *ark*), no such article — harmless here (falls to tier 2) but wrong upstream.

2026-07-20 · **The worktree-junction footgun DID fire on this machine — main's checkout was destroyed.** Correcting the record on the worktree-junction fix (ported separately on branch `fix/worktree-junction-footgun`), whose own STATE entry described the hazard as "not yet triggered by an incident here" — it had already fired: at session start `git status` in main showed **221 tracked files deleted**, `api/` and `web/` empty, `node_modules/.bin` emptied. Exactly the documented failure mode (a worktree teardown followed the junction into main). Recovered with `git restore .` + `git pull --ff-only` — **no uncommitted work was lost** (all 221 changes were pure deletions, zero modifications, zero untracked). **Still-live hazard:** 3 of the 4 existing worktrees (`dreamy-leakey`, `elated-dhawan`, `suspicious-vaughan`) were created by the OLD junctioning script and still junction main's `node_modules`; `recursing-hopper` has none. Until they're retired via `worktree-cleanup.ps1`, a manual recursive delete of any of those 3 re-wipes main. **Second-order damage not yet repaired:** `git restore` brings back tracked files but NOT `node_modules` — `vite` is missing and `npm install` fails with `ENOTEMPTY` on `web/node_modules/@mui/icons-material/esm` because a sibling session holds those files open (12 node processes live; per house rule, nothing was killed). Consequences: `npm run typecheck` cannot run (no `tsc` in `.bin`), and no dev server can start from main, so **browser verification is blocked** until an install succeeds during a quiet moment. **Lesson (see Lessons learned): a typecheck that reports "clean" against a damaged `node_modules` may be checking nothing** — `types: ["vite/client"]` failing to resolve made tsc silently skip all of `web/src`. A deliberate canary error is the cheap way to prove a checker is actually running; doing so here exposed a real type bug (`useCatalogs` declaring `Record & Map`) that the broken check had hidden.

2026-07-20 · **Ported worktree-junction footgun fix from downstream fork (deferredreward/bible-editor-multilingual#55).** That fork hit the same `worktree-init.ps1` design this repo had: junctioning `node_modules` (+ `web`/`api` node_modules) from main into each worktree. On Windows, a recursive delete of a worktree (`rm -rf` / `Remove-Item -Recurse` / `git worktree remove --force`) FOLLOWS the junction and deletes main's real files — main's `node_modules`, and transitively via npm's `node_modules/@bible-editor/{web,api}` workspace links, main's `web/` and `api/` **source**. This repo's `worktree-init.ps1` was byte-identical to their pre-fix version, so the same landmine was live here too. **Ported (not yet triggered by an incident here):** `worktree-init.ps1` now runs a real `npm install` per worktree (no junction to main; unlinks any legacy junction first if re-run in an old worktree) — matches upstream verbatim, package scope (`@bible-editor/{web,api}`) and `.claude/worktrees/` layout both confirmed identical. New `scripts/worktree-cleanup.ps1` classifies worktrees SAFE/GRAY/KEEP and safely removes them (unlinks junctions before any recursive delete; `-Remove` refuses anything but a known worktree/orphan). CLAUDE.md updated to require `worktree-cleanup.ps1` for teardown. **Not ported:** the fork's local nightly scheduled task (machine-local, lives outside their repo) — not set up here. **Not yet run:** the cleanup script hasn't been exercised against this machine's 4 live worktrees (`dreamy-leakey`, `elated-dhawan`, `recursing-hopper`, `suspicious-vaughan`, all created by the OLD junctioning `worktree-init.ps1` and so still carrying junctions to main) — recommend a dry-run pass to confirm classification before relying on it, and note those 4 still need `worktree-cleanup.ps1` (not manual deletion) for teardown until they're gone.
