...
- **The `deferredreward/bible-editor-multilingual` downstream fork is mostly not a source of unmerged fixes for
  ...
  worth a real diff, not just their sync-doc summary.** `highlight.ts` stayed byte-identical to ours for long
  stretches, so their bug-fix commits against it are directly portable even though the rest of the fork has
  rearchitected into a multi-tenant product with no equivalent surface here. **Next time this routine runs:**
  diff `web/src/lib/highlight.ts` (and any other shared, non-multi-tenant `lib/` module) directly against fork
  commits — don't trust their `docs/upstream-sync-*.md` "not applicable" verdicts alone, since that doc is
  written from their side of the diff, not ours. Open follow-up from the 2026-08-31 sync: issue #673.
