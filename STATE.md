...
- **Honesty ledger:** examples reach the AI only once the context-repo export + assisted mode exist (designed, deferred — blocked on `CONTEXT-REPO-CONTRACT.md`); QA-rules + templates UIs deferred (backend tables exist); tW key-term seeding deferred; register/terminology actually reaching uw-bt-bot unconfirmed (their steering is undocumented). Branch was rebase-pending onto `origin/main` (`2fc0824`, resource-labels PR #10 — only additive `api.ts` overlap) when the classifier outage interrupted; finish rebase + browser smoke before PR.
