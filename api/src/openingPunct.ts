// Mirror of web/src/lib/openingPunct.ts (issue #778). api/ cannot import web/,
// so this is a deliberate duplication — the same pattern as
// api/src/alignmentDelta.ts, whose header says the same thing. Keep behavior in
// sync: the web copy runs at the end of smartEditVerse (the in-app edit path),
// this copy is what the AI pipeline self-heal chain, the lint check and the
// repair script use, and all three must agree on what the defective shape is.
//
...
// trailing text child of the PRECEDING milestone (replace.ts relayoutUnchangedWords'
...
// web/src/lib/replace.test.mjs, whose `,\n‘` gap is a TOP-LEVEL text node
// between two milestones) and which the web edit engine already prefers for the
// verse-LEADING gap (see `topLevelIndexOf` in web/src/lib/replace.ts: "splice a
// verse-leading gap OUTSIDE the first milestone … rather than burying the
// opening quote inside the first \zaln").
//
...
//
// web/src/lib/openingPunct.ts is the counterpart copy, applied at the end of
// smartEditVerse. Keep the two in sync.
 ...
 * reconcileMarkers / splitGapAtMarker in web/src/lib/replace.ts, not here.
 */
