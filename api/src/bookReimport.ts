...
//
// This deliberately does NOT consult shouldRecordResourceSync's locked-chapter
// withholding (the EZK 40 lesson: "a watermark must not certify data it didn't
// apply"), and that is sound here rather than an oversight. That gate exists
// because a chapter skipped for a held pipeline lock leaves D1 BEHIND master.
// In this branch master's whole-file bytes are our own render of the whole
// book, so master demonstrably holds nothing D1 lacks — there is no chapter,
// locked or not, whose content we failed to apply. D1 may be AHEAD of master
// (app edits since pushed_read_at), which is the normal state and is what
