...
//      pending delete nightly. Since issue #427 option 1 shipped (#506), a
//      reissued tombstone is reclaimed automatically; this counter now fires
//      only when that reclaim itself lost its version-CAS race, so it withholds
//      on a narrower case than when this gate was written.
//
...
// shipped (#506), which is why this counter now fires far less often than when
// this gate was written: it withholds on the reclaim's lost-CAS-race case, not
// on every reissue. Either way, this gate makes a drop visible and stops the
// run from claiming the resource is current.
//
