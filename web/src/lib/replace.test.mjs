...
`;
  ...
  // (empty) \d node. NOTE (F2, #666 follow-up): usfm-js never nests a
  // following \zaln-s under \d — verified by inspecting the parsed tree for
  // every spacing variant tried, it always comes out as `{tag:"d"}` (no
  // text/children) with the milestone as a top-level SIBLING. That means this
  // sub-case never exercises the tag-only `\d` render gate at all: the zaln
  // renders as an ordinary top-level milestone regardless of how (or whether)
  // \d is gated, so this assertion is byte-identical pre- and post-#384-fix.
  // It is kept as a does-not-regress pin on the aligned-superscription shape,
  // NOT as proof of the #384 fix — that proof is the unaligned case above,
  // where the superscription text lives directly on the \d node itself.
...
`;
  ...
  assert(shownA.includes("A psalm of David"), `aligned superscription renders (does-not-regress pin, not a #384 proof — got ${JSON.stringify(shownA)})`);
  ...
  assert(
    ...
    `aligned displayed textContent matches baseline (does-not-regress pin, not a #384 proof — displayed ${JSON.stringify(normalizeEditable(textContentOf(editableA)))} vs baseline ${JSON.stringify(baselineA)})`,
  );
}
