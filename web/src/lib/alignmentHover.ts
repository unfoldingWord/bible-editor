...
// posToGroupId (which is display-derived) — hovering the Hebrew lit the card's
...
export interface HoverPosMaps {
  // position → which CARD owns it.
  posToGroupId: Map<number, string>;
...
export function makeHebrewHover(
  ...
  return {
    kind: "hebrew",
    pos,
    groupId: groupIdOverride ?? ctx.posMaps.posToGroupId.get(pos - ctx.posOffset) ?? null,
  };
}
...
export function resolveEnglishHighlight(
  ...
  if (hover.kind === "hebrew") {
    ...
    if (ctx.posMaps.posToGroupId.get(hover.pos - ctx.posOffset) === myGroupId) return "linked";
  ...
  return hover.positions.some((p) => ctx.posMaps.posToGroupId.get(p - ctx.posOffset) === myGroupId)
    ? "linked"
    : null;
}
...
export function resolveHebrewHighlight(
  ...
  const myGroupId =
    groupIdOverride ??
    (pos >= 0 ? ctx.posMaps.posToGroupId.get(pos - ctx.posOffset) ?? null : null);
  if (!myGroupId) return null;
  if (hover.kind === "hebrew") {
    ...
    return ctx.posMaps.posToGroupId.get(hover.pos - ctx.posOffset) === myGroupId ? "linked" : null;
  }
