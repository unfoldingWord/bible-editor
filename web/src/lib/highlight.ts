...
// TAG alone, not `type:"section"`, to match the renderer (see the walk()
// `o["tag"] === "d"` comment): usfm-js 3.5.0 parses a real `\d` as
// `{tag:"d", text}` with NO type, so requiring `type:"section"` here matched
// nothing usfm.toJSON emits and left render and matchers disagreeing —
// findTargetHighlights/extractTargetSelectionText returned empty for a node
// the renderer painted words for.
