...
export function summarizeReimport(res: ReimportResponse): string {
  ...
  // and the sync declined to adopt the difference because it was only formatting
  // our own render round-trip produces. Calling that "unchanged" would assert an
  // equality nobody measured — and on a chapter the last export reflowed, every
  // verse can land here, leaving this list empty and the snackbar reporting "no
  // changes" for a pull that silently declined every verse.
  if (t.skipped_normalized) parts.push(`${t.skipped_normalized} unchanged apart from formatting`);
