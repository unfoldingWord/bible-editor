// VERBATIM MIRROR of api/src/twHeadword.ts — keep the shared functions
// (generateVariants, headwordTermsFromTitle, matchesHeadword, FUNCTION_WORDS,
// isFunctionWord) byte-identical with the server so the TWL suggestion matcher
// and TWL ordering code agree on one definition of "does this English word
// correspond to this TW article headword", whether run server-side or in the
// browser. Precedent for an api↔web verbatim mirror: web/src/lib/usfmFormat.ts,
// web/src/lib/twlCanonicalOrder.ts. If you change one, change both.
//
...
// truth; api/src/twHeadword.ts is the primary copy — keep both in sync.
//
