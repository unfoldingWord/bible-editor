...
> Deployed to `https://bible-editor-api.unfoldingword.workers.dev` (Cloudflare Workers, unfoldingWord account). The default env in `api/wrangler.toml` carries dev-friendly values for `wrangler dev` and is named `bible-editor-api-dev` with **no crons**, so a plain `wrangler deploy` lands on a separate dev worker instead of overwriting prod; prod (worker name `bible-editor-api`, crons registered) lives under `[env.production.*]` and ships via `wrangler deploy --env production`. Any `--remote` D1 / `wrangler secret` / `wrangler tail` command needs `--env production` to target the deployed worker. `[env.production]` sets `account_id` (the unfoldingWord account, `5a3ffd86280d3ed086be76d955829242`) directly, so `--env production` commands resolve it without needing `CLOUDFLARE_ACCOUNT_ID` in the environment — this matters whenever the deploying OAuth token is a member of more than one Cloudflare account, which otherwise makes wrangler abort non-interactively. The default (dev) env has no `account_id` set; if a default-env `--remote` command (e.g. `wrangler d1 ... --remote` without `--env production`) hits the same ambiguity, set `CLOUDFLARE_ACCOUNT_ID` for that command explicitly rather than adding a top-level `account_id` — `bible_editor_dev`'s account membership hasn't been confirmed.
...
- **To see what sibling sessions are doing, ask git and GitHub, not a file:** run `scripts/worktrees.ps1` (read-only) — it lists every worktree led by *what it actually contains*, joining branch + open/merged PR + `.claude/state` headline + commit state in one table. Underneath it's just `git worktree list` and `gh pr list`, which you can also run raw; those are always accurate, unlike the tracked status section this replaced (when that was removed, all seven PRs it listed as "in progress" had already merged).
- **Worktree folder names are historical and frequently wrong — never infer a worktree's purpose from its directory name.** The folder and the branch are both minted by the harness, independently and at different times, and *neither* is reliably honest: folder `signout-button-placement-2e2751` held the internal-comments feature (PR #374), while folder `d1-incomplete-load-a1ac62` was accurate and its branch `claude/dreamy-jemison-d2eb50` was the meaningless one. The folder cannot be named correctly at birth because it exists before the branch does, and renaming it later would break any running dev server, editor window, or sibling session pinned to that path. `scripts/worktrees.ps1` exists to answer "which worktree is this?" so nobody has to guess from the name.
...
Read-only prod queries (from repo root): `node scripts/d1-select.mjs [--out <file>] "<one SELECT>"`. It refuses anything but a single SELECT / WITH…SELECT / EXPLAIN and is allowlisted in `.claude/settings.json`, so agents use it instead of raw `wrangler d1 execute --remote --env production`.
