...
// (reimportClassify.ts isReimportableRow), so a repair that left it NULL
// would be silently reverted from master the next night the tn file changed
// upstream — before the export could ever push the fix. Stamping the actor
// makes the row "edited", which is exactly what it now is.
//
...
// published book is locked by default.
//
// The published-book fallback alone is NOT enough to write against: an admin
// can lock an unpublished book with an explicit row, and without the dump we
// cannot see it. So --repair refuses outright when BOOK_LOCKS is unset,
// missing, or not shaped like a book_locks dump. Report mode still runs on
// the fallback, with a warning, because a report cannot write to anything.
const locksArg = process.env.BOOK_LOCKS ?? "";
let lockRows = null;
if (locksArg && existsSync(resolve(repoRoot, locksArg))) {
  lockRows = rowsOf(locksArg);
  ...
  if (doRepair) {
    console.error(
      "refusing to emit SQL: BOOK_LOCKS is unset or unreadable. --repair needs the real book_locks dump " +
        "(see the header) so an admin-locked unpublished book is never written to.",
    );
    process.exit(1);
  }
...
if (doRepair && !(Number.isInteger(actor) && actor > 0)) {
  console.error("refusing to emit SQL: REPAIR_ACTOR must be a users.id (e.g. REPAIR_ACTOR=2)");
  process.exit(1);
}

const sqlEscape = (s) => s.replace(/'/g, "''");

// Used only for the row-identifying comment line (book/chapter/verse/id — all
// fixed-format columns), NOT for the quote text itself: #770 found wrangler's
// client-side statement splitter was never proven safe against an apostrophe
// or semicolon inside a `-- comment`, and a free-text quote (raw Hebrew/Greek)
// is exactly the kind of value that could carry either. The old quote is no
// longer printed as a comment at all — it lives in the machine-readable
// sidecar (repair-tn-quotes.json, `oldQuote`) that rollback-tn-quotes.mjs
// already reads.
//
...
//
// updated_by is stamped with REPAIR_ACTOR so the row stops being "pristine"
// to the nightly DCS→D1 sync (see header). Without it the sync would write
// master's broken quote back over the repair before the export pushed it.
...
const lines = [
  ...
  `-- run: ${generatedAt}${bookFilter ? ` BOOK=${bookFilter}` : ""} — the rollback sidecar carries the same stamp and the two must match.`,
  `-- ${repairable.length} row(s) here, ${manual.length} left for manual review.`,
  "",
];
...
const unusable = repairable.filter((f) => !(Number.isInteger(f.row.version) && f.row.version >= 1));
if (unusable.length) {
  console.error(`
...
// Both files are overwritten by every --repair run, so a BOOK-filtered run
// after an unfiltered one leaves a sidecar that cannot undo the earlier SQL.
// The shared run stamp lets the rollback (and the operator) see the mismatch
// instead of silently generating no-op UPDATEs.
writeFileSync(
  sidecar,
  JSON.stringify(
    {
      ...
      rows: repairable.map((f) => ({
        ...
        oldVersion: f.row.version, oldQuote: f.row.quote, newQuote: f.verdict.suggestion,
      })),
    },
    null,
    2,
